	...
	<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
		...
		<div
			...
			<h5 class="mb-0 fw-bold">
				<i class="bi bi-bank2 me-2"></i> QRScanner List
			</h5>
			<div class="d-flex gap-2">
				<a href="<%=ORSView.QR_SCANNER_REPORT_CTL%>" target="_blank"
					class="btn btn-sm btn-warning fw-semibold"> <i
					class="bi bi-file-earmark-pdf me-1"></i> Print PDF
				</a> <a href="<%=ORSView.QR_SCANNER_REPORT_CTL%>?type=doc" target="_blank"
					class="btn btn-sm btn-info fw-semibold"> <i
					class="bi bi-file-earmark-word me-1"></i> Print DOC
				</a> <a href="QRScannerCtl"
					class="btn btn-sm btn-light text-primary fw-semibold"> <i
					class="bi bi-plus-circle me-1"></i> Add qrScanner
				</a>
			</div>
		</div>

		<form action="<%=ORSView.QR_SCANNER_LIST_CTL%>" method="POST">
			...
			<div
				...
				<input type="text" name="qrCode" class="form-control form-control-sm"
					style="max-width: 200px;" placeholder="Search by QrCode"
					value="<%=ServletUtility.getParameter("qrCode", request)%>">
					
				<input type="text" name="scannedBy" class="form-control form-control-sm"
					style="max-width: 180px;" placeholder="Search by ScannedBy"
					value="<%=ServletUtility.getParameter("scannedBy", request)%>">
				...
				<table class="table table-hover align-middle mb-0">
					...
					<tbody>
						...
						<tr>
							...
							<td><%=bean.getScannedBy()%></td>
							<td><%=bean.getScanTime()%></td>
							<td><%=bean.getStatus()%></td>
							
							<td><a href="QRScannerCtl?id=<%=bean.getId()%>"
