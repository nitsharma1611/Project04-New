...
public class UserModel extends BaseModel<UserBean> {
	...
	public List getRoles(UserBean bean) throws ApplicationException {
		...
		try {
			...
			log.error("Database Exception..", e);
			throw new ApplicationException(
					"Exception : Exception in get roles");
	...
	public boolean changePassword(Long id, String oldPassword,
		...
		try {
			...
			msg.setSubject(
					"ORS Password has been changed Successfully.");
	...
	public long registerUser(UserBean bean, String imagePath)
		...
		try {
			...
			msg.setSubject(
					"Registration is successful for ORS Project Rays Technologies");
			...
			log.error(
					"Email Server error, your emails may get delay",
					e);
		}
	...
	public boolean resetPassword(UserBean bean)
		...
		String newPassword =
				String.valueOf(new Date().getTime()).substring(0, 4);
	...
	public String getWhereClause(UserBean bean) {
		...
		if (bean != null) {
			...
			if (bean.getDob() != null
					&& bean.getDob().getDate() > 0) {

				sql.append(
						" AND DOB = " + bean.getGender());
			}
