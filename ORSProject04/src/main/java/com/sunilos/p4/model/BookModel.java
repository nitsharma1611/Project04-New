...
public class BookModel extends BaseModel<BookBean> {
	...
	public long add(BookBean bean) throws ApplicationException, DuplicateRecordException {
		...
		BookBean existBean = findByTitle(bean.getTitle());

		if (existBean != null) {
			throw new DuplicateRecordException("Book Title already exists");
		}
		...
		try {
			...
			throw new ApplicationException("Exception : Exception in add Title ");
		} finally {
	...
	public void update(BookBean bean) throws ApplicationException, DuplicateRecordException {
		...
		BookBean beanExist = findByTitle(bean.getTitle());

		// Check if updated College already exist
		if (beanExist != null && beanExist.getId() != bean.getId()) {

			throw new DuplicateRecordException("Book Title is already exist");
		}
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("UPDATE " + getTable()
					+ " SET title=?,author=?,price=?,publicationYear=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? WHERE ID=?");
		...
		pstmt.setString(5, bean.getCreatedBy());
			...
			throw new ApplicationException("Exception in updating Book Title ");
		} finally {
			JDBCDataSource.closeConnection(conn);
		}
	...
	public String getWhereClause(BookBean bean) {
		...
		if (bean != null) {
			...
			if (bean.getPublicationYear() != null && bean.getPublicationYear() > 0) {
